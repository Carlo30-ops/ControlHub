...
import { CHART_COLORS, MONTHS_ORDER } from "../../constants";
import { QuickActionCard } from "../../components/shared/QuickActionCard";
import { DashboardData, calcChange } from "./utils";
import { renderCustomPieLabel, CustomTooltip } from "./ChartComponents";
              ...
              <PieChart>
                ...
                >
                  {data.companyPieData.map((_entry, index) => (
                    <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} className="stroke-none outline-none opacity-80 hover:opacity-100 transition-opacity" />
                  ))}
                </Pie>
                <Tooltip
                  contentStyle={tooltipStyle}
                  formatter={(value: number, _name: string, props: { payload?: { fullName?: string; name?: string }; name?: string }) => [
                    `${value.toLocaleString("es-CO")} facturas`,
                    props.payload?.fullName ?? props.name ?? _name
                  ]}
                />
              </PieChart>
            </ResponsiveContainer>
